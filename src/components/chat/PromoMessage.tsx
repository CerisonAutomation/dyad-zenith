...
export function shouldShowPromoMessage({
  ...
  // Pro is always enabled — community promos only show for non-test users with messages
  return promoSeed !== null && !settings?.isTestMode && messagesLength > 0;
}
...
export function PromoMessage({ seed }: { seed: number }) {
  ...
  const isProPromo = false; // Pro trial dialog removed — community promos only

  const handleCtaClick = () => {
    posthog?.capture("promo_click", { messageId: message.id });
    if (message.target.type === "url") {
