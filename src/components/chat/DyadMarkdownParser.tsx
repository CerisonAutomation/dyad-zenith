...
export const DyadMarkdownParser: React.FC<DyadMarkdownParserProps> = ({
  ...
  // Older transcripts (stock Dyad builds) stored tool-call XML HTML-escaped
  // (e.g. `&lt;dyad-read path="x"&gt;`). Decode the entity only when it opens a
  // recognizable tag so those messages render as cards instead of raw text.
  const decodedContent = decodeEscapedDyadTags(isStreaming ? deferredContent : content);
  const contentToParse = decodedContent;
  ...
  return (
    <ComponentErrorBoundary name="DyadMarkdownParser">
      <>
        <MemoClosedBlocks
          blocks={closedBlocks}
...
// Decode `&lt;`-escaped dyad/think/tool_call tag openers from legacy
// stock-app transcripts so they parse as custom-tag blocks. Only the opening
// entity is decoded; attribute entities are handled by the parser itself.
function decodeEscapedDyadTags(content: string): string {
  return content.replace(
    /&lt;(\/?(?:dyad-[a-z-]+|think|tool_call|function|parameter)(?=[\s/>]))/gi,
    "<$1",
  );
}
...
function renderCustomTag(
  ...
  switch (tag) {
    ...
    case "dyad-exec":
    case "dyad-bash":
      // Legacy stock-app command tags — render as a script card.
      return (
        <DyadScript
          node={{
            properties: {
              description: attributes.description || attributes.command || "",
              truncated: attributes.truncated || "",
              executionMs: attributes["execution-ms"] || "",
              fullOutputPath: attributes["full-output-path"] || "",
            },
