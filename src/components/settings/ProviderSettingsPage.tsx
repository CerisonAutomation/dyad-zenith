...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  // isDyad: true for the built-in "auto" provider OR any provider with a gateway prefix
  // (gateway prefix means the provider routes through Dyad's gateway and has Pro features)
  const isDyad = provider === "auto" || !!(providerData as any)?.gatewayPrefix;
  ...
  const handleSaveKey = async (
    ...
    try {
      ...
      // Check if this is the first time the user is setting up Pro (any provider)
      const isNewDyadProSetup = settings && !hasDyadProKey(settings);
      ...
      // Pro features unlock dynamically for ANY default model/provider:
      // saving a key for the selected provider enables Pro.
      ...
      // Refetch user budget when a Pro-unlocking key is saved (any provider)
  ...
  useEffect(() => {
    setImageModelInput(
      settings?.imageGenerationProvider === provider
        ? (settings.imageGenerationModel ?? "")
        : "",
    );
  }, [provider, settings?.imageGenerationProvider, settings?.imageGenerationModel]);
  ...
  const canConfigureImageGeneration =
    provider === "openai" || providerData?.type === "custom";

  const handleSaveImageModel = async () => {
    const model = imageModelInput.trim();
    if (!model) {
      setSaveError("Enter an image-generation model name.");
      return;
    }
    setIsSaving(true);
    setSaveError(null);
    try {
      ...
      setTestSuccessMessage(
        `${providerDisplayName} / ${model} will be used for image generation.`,
      );
    } catch (error: any) {
    ...
    <div className="min-h-screen w-full">
      ...
      <div className="px-8 py-4">
        <div className="max-w-4xl mx-auto">
          ...
          {!settingsLoading && (
            <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
              ...
              <Switch
                ...
                checked={settings?.enableDyadPro ?? false}
          ...
          {canConfigureImageGeneration && !settingsLoading && settings && (
            <div className="mt-6 rounded-lg border bg-(--background-lightest) p-4">
              <div className="mb-3">
                <h3 className="font-medium">Image generation model</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Used by the Dyad generate_image tool. Custom providers must expose an OpenAI-compatible /images/generations endpoint.
                </p>
              </div>
              <div className="flex gap-2">
                <Input
                  value={imageModelInput}
                  onChange={(event) => setImageModelInput(event.target.value)}
                  placeholder={provider === "openai" ? "gpt-image-1" : "image-model-name"}
                  aria-label="Image generation model"
                />
                <Button
                  type="button"
                  onClick={() => void handleSaveImageModel()}
                  disabled={isSaving || !imageModelInput.trim()}
                >
                  Use for images
                </Button>
              </div>
