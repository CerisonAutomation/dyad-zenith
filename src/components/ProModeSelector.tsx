...
export function ProModeSelector() {
  ...
  // ADR-001: Pro features are unlocked in this custom build. The toggles must
  // work dynamically for ANY default model/provider — no key-detection gate.
  const proEnabled = settings ? isDyadProEnabled(settings) : false;
  const proModeTogglable = proEnabled;

  const showEnabledBadge = proEnabled;
        ...
        <div className="space-y-4">
          ...
          {showEnabledBadge && (
            <div className="text-sm text-center text-muted-foreground">
              <span className="inline-flex items-center justify-center gap-2 rounded-md border border-primary/30 bg-primary/10 px-3 py-2 text-sm font-medium text-primary shadow-sm transition-colors">
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <Switch
        id={id}
        aria-label={label}
        // Always show the REAL setting state. The old `isTogglable ? settingEnabled : false`
        // made every switch render OFF whenever the row was locked, which looked broken.
