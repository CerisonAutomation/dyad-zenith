...
- `mount()` creates a fresh Jotai store, seeds `selectedAppIdAtom` /
  ...
  route tree with a `/chat` route at `/chat?id=<chatId>&appId=<appId>`. The
  shared `useStreamChat` hook uses a non-throwing `/chat` search lookup so root
  and home surfaces can render safely when `/chat` is not active. It imports the
