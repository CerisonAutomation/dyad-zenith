...
const logger = log.scope("remote_client");
...
> implements RemoteActorRef<State, Event, Reason> {
  ...
  subscribe = (listener: () => void): (() => void) => {
    ...
    if (this.subscribers === 1) {
      ...
      void this.lease.ready.catch((error) => logger.debug("Suppressed error:", error));
    }
    let active = true;
    return () => {
      if (!active) return;
  ...
  bootstrap(): Promise<void> {
    ...
    if (active?.isCurrent()) {
      ...
      return active.promise.catch((error) => logger.debug("Suppressed error:", error)).then(() => this.bootstrap());
    }
      ...
      try {
        ...
        if (generation !== this.bootstrapGeneration || !this.hasInterest()) {
          if (!subscription.isCurrent() || !this.hasInterest()) {
            await subscription.release().catch((error) => logger.debug("Suppressed error:", error));
  ...
  private applySnapshot(snapshot: MachineSnapshotEnvelope): void {
    ...
    if (snapshot.revision !== this.metadata.revision + 1) {
      void this.resync().catch((error) => logger.debug("Suppressed error:", error));
      return;
    }
    const observedRevision = this.createObservedRevision(snapshot);
    this.metadata = {
...
export class RemoteMachineClient {
    ...
    for (const store of this.stores.values()) {
      if (store.hasInterest()) {
        void previousConnection
          ...
          .catch((error) => logger.debug("Suppressed error:", error));
      }
    }
  ...
  releaseTransport(store: RemoteSnapshotStore<any, any, any, any>): void {
    ...
    void this.connection.unsubscribe(store.address).catch((error) => logger.debug("Suppressed error:", error));
  }
    ...
    if (currentStatus === this.status) {
      for (const store of this.stores.values()) {
        ...
        if (currentStatus === "connected" && store.hasInterest()) {
          void store.bootstrap().catch((error) => logger.debug("Suppressed error:", error));
        }
      }
    } else {
