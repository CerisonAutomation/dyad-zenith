...
export function useStreamChat({
  ...
  // This hook is shared by RootLayout/Home/import/security surfaces that may
  // render when /chat is not active. A strict route-scoped lookup throws the
  // TanStack Router invariant "Could not find an active match from /chat" in
  // that state. Keep the hook unconditional, but make the exact /chat lookup
  // non-throwing so we preserve route-specific typing without reading another
  // route's unrelated `id` search parameter.
  const searchResult = useSearch({ from: "/chat", shouldThrow: false });
  const chatId = hasChatId ? searchResult?.id : undefined;
