...
const mocks = vi.hoisted(() => ({
  ...
  showError: vi.fn(),
  searchResult: { current: { id: 42 } as { id: number } | undefined },
...
vi.mock("@tanstack/react-router", () => ({
  useSearch: (options: Record<string, unknown>) => {
    mocks.searchOptions.push(options);
    if (
      mocks.searchResult.current === undefined &&
    ...
    return mocks.searchResult.current;
  },
}));
...
describe("useStreamChat main-owned queue", () => {
  beforeEach(() => {
    ...
    mocks.searchResult.current = { id: CHAT_ID };
    mocks.searchOptions.length = 0;
  });

  it("does not require /chat to be active for shared consumers", () => {
    const { Wrapper } = makeWrapper();
    mocks.searchResult.current = undefined;

    expect(() =>
      renderHook(() => useStreamChat({ hasChatId: false }), { wrapper: Wrapper }),
    ).not.toThrow();

    expect(mocks.searchOptions.at(-1)).toMatchObject({
      from: "/chat",
      shouldThrow: false,
    });
  });
