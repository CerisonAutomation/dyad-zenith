...
console.log("[BOOT] main.ts loaded, E2E_TEST_BUILD=", process.env.E2E_TEST_BUILD);
...
// Suppress EPIPE errors when stdout/stderr pipe is broken (e.g. terminal closed).
// electron-log's console transport writes to process.stdout/stderr which can
// become unavailable when the parent process detaches or the terminal closes.
// Override the console transport's writeFn to catch EPIPE at the source.
  ...
  if (typeof originalWriteFn === "function") {
    (consoleTransport as any).writeFn = (args: any) => {
      try {
        ...
        if (err?.code === "EPIPE") {
          // Pipe broken — silently ignore, file transport still works
          return;
        }
        throw err;
      }
...
// Also catch any EPIPE that slips through uncaughtException
...
export async function onReady() {
  ...
  debugLog(
    "getManagedNodeVersion done, version=" + String(managedNodeVersion),
  );
...
const createWindow = ({
  ...
  debugLog("createWindow: BrowserWindow created, id=" + String(browserWindow.id));
  ...
  if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
    ...
    // is fully listening.
    const waitForDevServer = async (url: string, retries = 10, delayMs = 1000): Promise<void> => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        ...
        if (attempt < retries) {
          debugLog(`Waiting for Vite dev server (attempt ${attempt}/${retries})...`);
...
async function createFreshStartupWindow(): Promise<void> {
  ...
  debugLog(
    "createFreshStartupWindow: isAppQuitting=" + String(isAppQuitting),
  );
  if (isAppQuitting) {
