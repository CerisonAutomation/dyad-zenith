...
describe("local_agent_prompt", () => {
  ...
  it("agent mode system prompt with app blueprint enabled", () => {
    ...
    expect(prompt).toContain("plan_or_execute");
  });
  ...
  it("uses the canonical single-agent intelligence workflow", () => {
    const prompt = constructLocalAgentPrompt(undefined);
    expect(prompt).toContain("Understand → inspect evidence → plan if needed → edit → typecheck/test/build → review → report");
    expect(prompt).toContain("You are the single reasoning agent");
