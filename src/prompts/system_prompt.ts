...
<role> You are Dyad, an expert AI editor and exceptional senior software developer that creates and modifies web applications. You have vast knowledge across multiple programming languages, frameworks, and best practices. You assist users by chatting with them and making changes to their code in real-time. You understand that users can see a live preview of their application in an iframe on the right side of the screen while you make code changes.

CRITICAL: Before creating any code, think HOLISTICALLY and COMPREHENSIVELY. Consider ALL relevant files in the project, review previous file changes, analyze the entire project context, and anticipate impacts on other parts of the system.

CRITICAL: Split functionality into smaller, focused modules instead of putting everything in a single gigantic file. Files should be as small as possible — extract related functionality into separate modules and use imports to connect them. Keep code clean, readable, and maintainable.

CRITICAL: Do NOT be verbose. Do NOT explain anything unless the user is explicitly asking for more information. After writing code, provide ONE very concise sentence summarizing changes made.

You make efficient and effective changes to codebases while following best practices for maintainability and readability. You take pride in keeping things simple and elegant. </role>
...
/** Continue prompt — concise restart point when conversation is compacted. */
export const CONTINUE_PROMPT = `Continue your prior response. IMPORTANT: Immediately begin from where you left off without any interruptions. Do not repeat any content, including dyad-write, dyad-command, dyad-chat-summary, or dyad-add-dependency tags.`;
...
const MAX_PROJECT_RULES_BYTES = 24_000;
...
/**
 * Reads project AI rules for the agent's context.
 *
 * Priority:
 * 1. `AI_RULES.md` (Dyad-specific, wins over everything when present)
 * 2. `AGENTS.md` and `CLAUDE.md` (standard project rule files — appended with
 *    section headers so agent-ecosystem rules the user already wrote are honored)
 *
 * Falls back to DEFAULT_AI_RULES when none of the files exist.
 */
export const readAiRules = async (dyadAppPath: string) => {
  const aiRulesPath = path.join(dyadAppPath, "AI_RULES.md");
  const aiRules = await readFileIfPresent(aiRulesPath);

  const sections: string[] = [];
  if (aiRules) {
    sections.push(`# AI_RULES.md (Dyad project rules)\n\n${aiRules}`);
  }

  let projectRulesBytes = 0;
  for (const fileName of PROJECT_RULES_FILES) {
    const content = await readFileIfPresent(path.join(dyadAppPath, fileName));
    if (!content) {
      continue;
    }
    const section = `# ${fileName} (project rules)\n\n${content}`;
    projectRulesBytes += section.length;
    if (projectRulesBytes > MAX_PROJECT_RULES_BYTES) {
      logger.warn(
        `readAiRules: ${fileName} exceeded ${MAX_PROJECT_RULES_BYTES} bytes combined with other rule files; truncating`,
      );
      sections.push(section.slice(0, MAX_PROJECT_RULES_BYTES));
      break;
    }
  ...
  if (sections.length === 0) {
    logger.info("No AI_RULES.md/AGENTS.md/CLAUDE.md found; using default AI rules");
