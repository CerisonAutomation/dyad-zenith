...
2. **Prioritize recent changes**: Weight the latter part of the conversation more heavily
3. **Use exact file paths**: Always use full file paths when referencing code
4. **Capture intent**: Include the "why" behind decisions, not just the "what"
5. **Preserve errors**: If debugging, carry the exact error message forward
6. **Preserve plan references**: If a plan was created or updated, include its title, current status, and remaining steps
7. **Preserve standing preferences**: A constraint stated once, even early, still binds — carry it forward even when the recent conversation never repeats it
8. **Skip empty sections**: Omit any section that has no content`;
