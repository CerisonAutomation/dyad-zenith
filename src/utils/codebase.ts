...
// Local smart-context token budget: how much of the codebase to include in
// the prompt when the Dyad engine (which would do server-side filtering) is
// not available. 90K leaves ~60K for system prompt, history and output on
// a 200K-window model.
const LOCAL_SMART_CONTEXT_TOKEN_BUDGET = 90_000;
...
async function prepareCodebaseFiles({
  ...
  // --- Local smart context (no Dyad engine key) ---
  // When Pro smart-context is enabled but there is NO real Dyad engine key
  // (the engine rejects provider keys with 401), apply a LOCAL relevance
  // filter: always keep forced/selected files, then add the most recently
  // modified files until the token budget is reached. This prevents
  // context-window overflow without the engine.
  const hasRealDyadKey = Boolean(
    settings.providerSettings?.auto?.apiKey?.value,
  );
  const useLocalSmartContext =
    isSmartContextEnabled && !hasRealDyadKey && files.length > 0;
  let cappedFiles = sortedFiles;
  if (useLocalSmartContext) {
    cappedFiles = await capFilesToTokenBudget({
      files: sortedFiles,
      forceSet: autoIncludedFiles,
      budgetTokens: LOCAL_SMART_CONTEXT_TOKEN_BUDGET,
    });
    logger.info(
      `[codebase] Local smart context: ${sortedFiles.length} -> ${cappedFiles.length} files (budget ${LOCAL_SMART_CONTEXT_TOKEN_BUDGET} tokens)`,
    );
  }

  return cappedFiles.map((file) => ({
    absolutePath: file,
...
/**
 * Cap a file list to a token budget. Forced files are always kept; the
 * remaining files (already sorted by modification time, most recent first)
 * are included until the budget is exhausted. Token estimate: bytes / 4.
 */
async function capFilesToTokenBudget({
  files,
  forceSet,
  budgetTokens,
}: {
  ...
  const result: string[] = [];
  let usedTokens = 0;
  const forced = files.filter((f) => forceSet.has(path.normalize(f)));
  const rest = files.filter((f) => !forceSet.has(path.normalize(f)));

  for (const file of forced) {
    result.push(file);
    try {
      const stat = await fsAsync.stat(file);
      usedTokens += Math.ceil(stat.size / 4);
    } catch {
...
async function extractCodebaseInner({
  ...
  // Cap the joined output so a huge codebase cannot blow the model context
  // window (observed: 12.7MB / ~3.2M estimated tokens for one app). Keep the
  // earliest files and mark truncation so the model knows the context is
  // partial and can fetch specific files on demand.
  const MAX_CODEBASE_CHARS = 400_000; // ~100K tokens — Headroom compresses before LLM
  let formattedOutput = "";
  let codebaseTruncated = false;
  for (const formatted of formattedFiles) {
    if (formattedOutput.length + formatted.length > MAX_CODEBASE_CHARS) {
      codebaseTruncated = true;
      break;
    }
    formattedOutput += formatted;
  }
  if (codebaseTruncated) {
    formattedOutput +=
      "\n\n<!-- ⚠ codebase context truncated at " +
      MAX_CODEBASE_CHARS +
      " chars (~40K tokens). Use read_file/explore_code to fetch specific files. -->\n";
    logger.warn(
      `extractCodebase: output truncated at ${MAX_CODEBASE_CHARS} chars (${formattedFiles.length} files total)`,
    );
  }
