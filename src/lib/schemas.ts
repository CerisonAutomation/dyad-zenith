...
export const RegularProviderSettingSchema = z.object({
  ...
  /** Optional OpenAI-compatible image model for this provider. */
...
const BaseUserSettingsFields = {
  ...
  /** Provider used by generate_image. Must be OpenAI-compatible for images. */
  imageGenerationProvider: z.string().optional(),
  /** Optional model override used by generate_image. */
...
export function hasDyadProKey(settings: UserSettings): boolean {
  return Boolean(settings.providerSettings?.auto?.apiKey?.value);
}

/**
 * The hosted Dyad engine credential is intentionally separate from BYOK
 * provider credentials. A Kilo/OpenRouter/OpenAI key can power the local agent
 * directly, but it must never be treated as a Dyad subscription credential.
 */
...
export function isDyadProEnabled(settings: UserSettings): boolean {
  // Custom build: Pro is unlocked via synthetic unlimited budget in
  // pro_handlers.ts. The API key gate (hasDyadProKey) is bypassed so
  // all Pro UI elements, tools, and features are visible without a
  // Dyad cloud subscription.
  return settings.enableDyadPro === true;
}

/**
 * Whether Agent mode has a model selection that can be resolved. The Auto
 * model is a first-class selection: getModelClient() performs free-first
 * provider discovery and returns an actionable setup error if no provider is
 * configured.
 */
export function hasConfiguredAgentModel(settings: UserSettings): boolean {
  return Boolean(settings.selectedModel?.provider && settings.selectedModel?.name);
}
