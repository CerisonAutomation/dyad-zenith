...
const logger = log.scope("register-renderer-ipc");
...
export function registerQueryInvalidationListener(
  ...
  const bootstrapInvalidations = () => {
    void ipcClient.windowInfrastructure
      ...
      .catch((error) => {
        if (disposed) return;
        logger.error("Failed to bootstrap window infrastructure", error);
...
export function registerRendererIpcListeners({
  ...
  const publishVisibleEntities = () => {
    ...
    void ipcClient.windowInfrastructure
      ...
      .catch((error) =>
        logger.error("Failed to publish visible window entities", error),
      );
  };
  const updateInterest = (
    ...
    if (previous !== null) {
      void ipcClient.windowInfrastructure
        ...
        .catch((error) =>
          logger.error(`Failed to detach ${kind} interest`, error),
        );
    }
    if (next !== null) {
      void ipcClient.windowInfrastructure
        ...
        .catch((error) =>
          logger.error(`Failed to attach ${kind} interest`, error),
        );
    }
  };
  ...
  const handleWindowFocus = () => {
    void ipcClient.windowInfrastructure
      .setFocused()
      .catch((error) => logger.error("Failed to publish window focus", error));
  };
    ...
    ipcClient.events.windowInfrastructure.onEntityDisposed(
      ({ entity, epoch }) => {
        ...
        try {
          ...
          logger.error("Window-local entity disposal failed", {
            entity,
            error,
          });
        }
