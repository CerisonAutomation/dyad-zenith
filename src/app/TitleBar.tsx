...
export const TitleBar = () => {
  ...
  const isDyadPro = settings ? isDyadProEnabled(settings) : false;
    ...
    <>
      <div className="@container z-11 w-full h-[calc(var(--layout-title-bar-offset)+1px)] pt-1 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <div className="flex items-center shrink-0">
          ...
          {isDyadPro && <DyadProButton isDyadProEnabled={isDyadPro} />}
        </div>
...
export function AICreditStatus({
  ...
  const resetDate = userBudget.budgetResetDate
    ...
    : null;
  const tooltipText = `${remaining.toLocaleString()} of ${total.toLocaleString()} credits remaining${resetDate ? ` · resets on ${resetDate}` : ""} · Note: credit status may take a moment to update.`;
  return (
    <span
      title={tooltipText}
      className="text-xs pl-1 mt-0.5 opacity-90 cursor-help"
    >
      · {remaining}
    </span>
  );
