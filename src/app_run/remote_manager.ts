...
const logger = log.scope("app-run-remote-manager");
  ...
  requestManualReload = (appId: number): void => {
    ...
    void (async () => {
      try {
        ...
        logger.error("[app-run] Preview reload dispatch failed:", error);
      }
    })();
  };
  ...
  private handleActorSnapshot(
    ...
    for (const listener of this.listeners) {
      try {
        ...
        logger.error("[app-run] Remote state listener failed:", error);
      }
    }
  }
}
