...
import { LOCAL_MCP_CATALOG } from "./local_mcp_catalog";
...
function parseEntries(raw: unknown[]): McpCatalogEntry[] {
  ...
  // Local keyless entries ship with the app and take precedence over the
  // same-slug remote entry (the remote catalog is a Dyad Engine dependency
  // and may be unreachable or offline).
  for (const candidate of LOCAL_MCP_CATALOG) {
    const result = McpCatalogEntrySchema.safeParse(candidate);
    if (!result.success) {
      logger.debug("Dropping local catalog entry that failed validation");
      continue;
    }
  ...
  for (const candidate of raw) {
    ...
    // Engine-replacement mode: remote-only entries come from the Dyad
    // Engine catalog (api.dyad.sh) and require a Dyad account/credits to
    // run (e.g. supabase, hosted servers). They fail with "Connection
    // closed" without the engine, so drop anything without a local
    // keyless counterpart. Local entries already won by slug above.
    if (!seen.has(result.data.slug)) {
      logger.debug(
        `Dropping remote-only catalog entry (no local keyless equivalent): ${result.data.slug}`,
      );
      continue;
    }
    seen.add(result.data.slug);
    ...
    catalogFetchPromise = (async () => {
      try {
        ...
        catalogCache = {
          entries,
          expiresAt:
            entries.length > LOCAL_MCP_CATALOG.length
            ? expiresAt
            : Date.now() + FAILURE_CACHE_TTL_MS,
        };
        return entries;
      } catch (error) {
