...
import { KILOCODE_AUTO_FREE_MODEL } from "./kilocode_gateway";
...
function buildFallbackCatalog(): BuiltinLanguageModelCatalog {
  ...
  return {
    ...
    aliases: [
      ...
      {
        id: "dyad/auto/kilocode",
        resolvedModel: {
          providerId: "kilocode",
          apiName: KILOCODE_AUTO_FREE_MODEL,
        },
        displayName: "Auto Kilo Free",
        purpose: "auto-mode",
      },
      {
...
function convertRemoteCatalog(
  ...
  // Defensive: never crash on a malformed remote payload. Missing/partial
  // fields fall back to empty collections — the merge below re-adds builtin
  // aliases and the auto fallback models, so the builtin catalog always wins.
  const providers = Array.isArray(remoteCatalog.providers)
    ? remoteCatalog.providers
    : [];
  const rawModelsByProvider =
    remoteCatalog.modelsByProvider &&
    typeof remoteCatalog.modelsByProvider === "object"
      ? remoteCatalog.modelsByProvider
      : {};
  const modelsByProvider: Record<string, CatalogModel[]> = Object.fromEntries(
    Object.entries(rawModelsByProvider)
      .filter(([, models]) => Array.isArray(models))
      .map(([providerId, models]) => [
        providerId,
        (models as CatalogModel[]).map((model) => ({
  ...
  const providerList: LanguageModelProvider[] = providers.map((provider) => ({
    id: provider.id,
    name: provider.displayName,
    hasFreeTier: provider.hasFreeTier,
    websiteUrl: provider.websiteUrl,
  ...
  // Remote catalogs are additive, not authoritative for local capabilities.
  // Preserve built-in providers (notably Kilo/Zenith Auto) even if an older
  // server-side catalog has not learned about them yet.
  const fallback = buildFallbackCatalog();
  const providerIds = new Set(providerList.map((provider) => provider.id));
  for (const fallbackProvider of fallback.providers) {
    if (!providerIds.has(fallbackProvider.id)) {
      providerList.push(fallbackProvider);
      providerIds.add(fallbackProvider.id);
    }
  }
  for (const [providerId, fallbackModels] of Object.entries(
    fallback.modelsByProvider,
  )) {
    if (!modelsByProvider[providerId]?.length) {
      modelsByProvider[providerId] = fallbackModels;
    }
  }
  ...
  const remoteAliases = Array.isArray(remoteCatalog.aliases)
    ? remoteCatalog.aliases
    : [];
  const remoteAliasIds = new Set(remoteAliases.map((a) => a.id));
  const mergedAliases = [
    ...remoteAliases,
...
async function fetchRemoteCatalog(): Promise<BuiltinLanguageModelCatalog | null> {
  ...
  try {
    ...
    // Defensive: never let a malformed remote response crash the refresh.
    // Validation failures fall back to the builtin catalog (which already
    // ships every provider incl. opencode with all 25 models).
    const parsedCatalog =
      LanguageModelCatalogResponseSchema.safeParse(rawCatalog);
    if (!parsedCatalog.success) {
      logger.warn(
        "Remote language model catalog failed validation; using builtin catalog",
        {
          issues: parsedCatalog.error.issues
            .slice(0, 5)
    ...
    let convertedCatalog: BuiltinLanguageModelCatalog;
    try {
      convertedCatalog = convertRemoteCatalog(parsedCatalog.data);
    } catch (convertError) {
      logger.warn(
        "Remote language model catalog conversion failed; using builtin catalog",
        convertError,
      );
      return null;
    }
