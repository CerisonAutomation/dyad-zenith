...
// Engine removed — image generation uses local/open providers.
...
export class ImageGenerationService {
  ...
  private async execute(
    ...
    // ADR-001: custom build — image generation prefers the user's configured
    // OpenAI-compatible provider (OpenCode first), then falls back to a
    // keyless open-source endpoint (Pollinations.ai) so it always works
    // without the Dyad Engine.
    const apiKey =
    ...
    const _rawImageBaseURL: string | undefined =
      (settings.providerSettings?.opencode as any)?.baseURL;
    const imageBaseURL = _rawImageBaseURL
      ? _rawImageBaseURL.replace(/\/$/, "")
      : "http://localhost:0/engine-removed";
    ...
    // Keyless open-source fallback (Pollinations.ai — free, no API key).
    const generateKeyless = async (): Promise<Buffer> => {
      ...
      pollinationsUrl.searchParams.set(
        "seed",
        String(Math.floor(Math.random() * 1_000_000)),
      );
      const imgResponse = await fetch(pollinationsUrl.toString(), {
        signal: controller.signal,
      });
      if (!imgResponse.ok) {
        throw new DyadError(
          `Keyless image generation failed (HTTP ${imgResponse.status}). Please try again.`,
      ...
      if (arrayBuffer.byteLength > MAX_IMAGE_SIZE) {
        throw new DyadError(
          "Generated image exceeds maximum allowed size",
    ...
    if (apiKey) {
      try {
        response = await fetch(`${imageBaseURL}/images/generations`, {
          method: "POST",
          headers: {
        ...
        logger.warn(
          "Provider image generation failed, falling back to keyless",
          error,
        );
      } finally {
    ...
    if (!response || !response.ok) {
      // Fall back to the keyless open-source endpoint so image generation
      // always works without the Dyad Engine.
      imageBuffer = await generateKeyless();
    } else {
