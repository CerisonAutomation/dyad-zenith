  ...
  private async createClient(serverId: number): Promise<MCPClient> {
    ...
    if (s.transport === "stdio") {
      ...
      const stdio = new StdioClientTransport({
        ...
        args,
        env,
        // Prevent a hung MCP server from blocking agent runs forever.
        stderr: "pipe",
      });
      // Connection timeout: if the server doesn't finish handshaking within
      // 30s, fail fast and surface a clear error instead of hanging.
      let timeoutId: ReturnType<typeof setTimeout> | undefined;
      client = await Promise.race([
        createMCPClient({ transport: stdio }),
        new Promise<never>((_, reject) => {
          timeoutId = setTimeout(
            () =>
              reject(
                new Error(
                  `MCP server ${s.id} (${s.name}) connection timeout after 30s`,
                ),
              ),
            30000,
          );
