...
describe.runIf(process.platform !== "win32")("resolveReadPathWithinApp", () => {
  it("allows in-project reads and rejects symlink escapes", async () => {
    const appPath = await fs.mkdtemp(path.join(os.tmpdir(), "dyad-read-"));
    const outsidePath = await fs.mkdtemp(path.join(os.tmpdir(), "dyad-outside-"));
    try {
      await fs.writeFile(path.join(appPath, "inside.ts"), "export const ok = true;\n");
      await fs.writeFile(path.join(outsidePath, "secret.ts"), "export const secret = true;\n");
      await fs.symlink(outsidePath, path.join(appPath, "outside"), "dir");

      await expect(
        resolveReadPathWithinApp({ appPath, relativePath: "inside.ts" }),
      ).resolves.toBe(await fs.realpath(path.join(appPath, "inside.ts")));
      await expect(
        resolveReadPathWithinApp({ appPath, relativePath: "outside/secret.ts" }),
