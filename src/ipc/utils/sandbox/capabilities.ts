...
async function assertResolvedPathAllowed(params: {
  ...
  try {
    ...
    if (isNodeErrorWithCode(error, "ENOENT")) {
      throw new DyadError(
        `File not found: ${params.displayPath}. Use list_files() to discover available files, or check if the path is correct.`,
        DyadErrorKind.NotFound,
      );
    }
    throw error;
...
/**
 * Safe alternative to file_stats("node_modules/pkg") — reads package.json only.
 * Returns whether the named package is listed in any dependency field, and the
 * version string if present. Does NOT require node_modules access.
 */
export async function sandboxCheckPackageInstalled(
  appPath: string,
  packageName: string,
): Promise<PackageInstallStatus> {
  if (!packageName || typeof packageName !== "string") {
    throw new DyadError("check_package_installed: packageName must be a string.", DyadErrorKind.Validation);
  }
  ...
  for (const source of ["dependencies", "devDependencies", "peerDependencies"] as const) {
    const version = pkg[source]?.[packageName];
...
export function buildSandboxCapabilitiesWithObserver(
  ...
  return {
    ...
    check_package_installed: (packageName: unknown) => {
      if (typeof packageName !== "string") {
        throw new DyadError(
          "check_package_installed: packageName must be a string.",
          DyadErrorKind.Validation,
        );
      }
      onHostCall?.({ name: "check_package_installed" });
      return sandboxCheckPackageInstalled(appPath, packageName);
    },
  };
}
