...
/**
 * Resolve an existing read target and prove that its real path remains inside
 * the project after following symlinks. This closes the gap left by lexical
 * containment checks alone: a project-local symlink must never become a read
 * tunnel to arbitrary host files.
 */
export async function resolveReadPathWithinApp(params: {
  appPath: string;
  relativePath: string;
}): Promise<string> {
  const joinedPath = safeJoin(params.appPath, params.relativePath);
  const [realAppPath, realTargetPath] = await Promise.all([
    fs.promises.realpath(params.appPath),
    fs.promises.realpath(joinedPath),
  ]);
  ...
  if (
    ...
    throw new DyadError(
      `Cannot read files outside the app: ${params.relativePath}`,
