...
describe("redactTelemetryText", () => {
  it("removes API keys, bearer tokens, and credential query values", () => {
    const source =
      "gsk_abcdefghijklmnopqrstuvwxyz123456 Bearer abc.def.ghi https://x.test/?token=supersecret123 api_key=anothersecret123";
    const result = redactTelemetryText(source) ?? "";
    expect(result).not.toContain("abcdefghijklmnopqrstuvwxyz123456");
    expect(result).not.toContain("abc.def.ghi");
    expect(result).not.toContain("supersecret123");
    expect(result).not.toContain("anothersecret123");
