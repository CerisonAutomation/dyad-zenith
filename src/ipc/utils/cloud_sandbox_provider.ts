...
// Engine removed — sandbox operations use local environment.
function getLocalSandboxUrl(): string {
  return "http://localhost:0/engine-removed";
}
...
function getDyadEngineApiKey() {
  ...
  // The Dyad engine only accepts Dyad-issued keys (the "auto" provider).
  // Provider keys (opencode, openai, ...) are rejected with a 401
  // "Invalid proxy server token", so ONLY the auto key is used here.
...
async function cloudSandboxFetch(
  ...
  const response = await fetch(`${getLocalSandboxUrl()}${endpoint}`, {
    ...init,
    headers,
  });
