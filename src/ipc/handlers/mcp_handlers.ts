...
export function registerMcpHandlers() {
  ...
  createTypedHandler(mcpContracts.updateServer, async (_, params) => {
    ...
    const mutate = async () => {
      ...
      void mcpManager.dispose(params.id).catch((error) => logger.debug("Suppressed error:", error));
      return toMcpServer(result[0]);
    };
    const oauthRelevant =
  ...
  createTypedHandler(mcpContracts.deleteServer, async (_, id) => {
    return await withMcpOAuthServerMutation(
      ...
      async () => {
        void mcpManager.dispose(id).catch((error) => logger.debug("Suppressed error:", error));
  ...
  createTypedHandler(mcpContracts.listTools, async (_, serverId) => {
    ...
    try {
      ...
      void mcpManager.dispose(serverId).catch((error) => logger.debug("Suppressed error:", error));
      const message =
  ...
  // Drives the no-keyring banner on the MCP settings page. Reports true
  // when either the OS keyring or the app-local AES key file can encrypt
  // stored credentials, so the banner only appears when secrets would
  // genuinely be stored as plaintext.
  createTypedHandler(mcpContracts.isOauthStorageEncrypted, async () => {
    return { available: isSecretEncryptionAvailable() };
  });
