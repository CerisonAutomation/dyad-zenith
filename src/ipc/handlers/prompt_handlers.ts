...
import { getGuideIndex } from "@/pro/main/ipc/handlers/local_agent/tools/read_guide";
...
export function registerPromptHandlers() {
  ...
  createTypedHandler(promptContracts.listGuides, async () => {
  ...
  createTypedHandler(promptContracts.create, async (_, params) => {
    ...
    if (!row) throw new DyadError("Failed to fetch created prompt", DyadErrorKind.Internal);
    return {
      id: row.id!,
      title: row.title,
  ...
  createTypedHandler(promptContracts.update, async (_, params) => {
    ...
    if (!id) throw new DyadError("Prompt id is required", DyadErrorKind.Validation);
    const now = new Date();
