...
async function handleCreateProject(
  ...
  try {
    ...
    try {
      ...
      if ("inspectorUrl" in deploymentData && deploymentData.inspectorUrl) {
        logger.info(
          `First deployment successful: ${deploymentData.inspectorUrl}`,
        );
      } else if ("id" in deploymentData) {
        logger.info(`First deployment successful: ${deploymentData.id}`);
      } else {
