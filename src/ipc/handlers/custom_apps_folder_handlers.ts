...
export function registerCustomAppsFolderHandlers() {
  ...
  createTypedHandler(systemContracts.setCustomAppsFolder, async (_, input) => {
    ...
    if (input) {
      ...
      if (!isAbsolute(input)) throw new DyadError("Directory path is not absolute", DyadErrorKind.Validation);

      // Make sure it exists
      if (!isDirectoryAccessible(input))
        throw new DyadError("Path is not a directory", DyadErrorKind.Validation);
