...
/** Returns true for localhost variants and RFC 1918 / loopback private ranges. */
...
export function registerLanguageModelHandlers() {
  ...
  handle(
    ...
    async (
      ...
      // Validate URL format and restrict to HTTPS (SSRF protection)
      let parsedUrl: URL;
      try {
      ...
      if (
        ...
        throw new DyadError(
          "API base URL must use HTTPS (except for localhost or private network addresses)",
          DyadErrorKind.Validation,
        );
      }
      ...
      // Return the newly created provider with the prefixed ID to match database
      return {
        id: CUSTOM_PROVIDER_PREFIX + id,
        name,
        apiBaseUrl,
  ...
  handle(
    ...
    async (
      ...
      if (!existingModel) {
        throw new DyadError(
          `Model with API name "${apiName}" not found`,
          DyadErrorKind.NotFound,
        );
      }
  ...
  handle(
    "auto-discover-language-models",
    async (): Promise<Record<string, LanguageModel[]>> => {
      return autoDiscoverModels();
    },
  );
}
