...
import { DEFAULT_AGENT_MODE, DEFAULT_AGENT_MODEL } from "@/agent/defaults";
...
export const DEFAULT_SETTINGS: UserSettings = {
  ...
  selectedChatMode: DEFAULT_AGENT_MODE,
  defaultChatMode: DEFAULT_AGENT_MODE,
...
export function readSettings(): UserSettings {
  try {
    ...
    if (isSafeStorageNotReadyError(error)) {
      // Expected at very early startup before the OS keyring is available;
      // the caller retries after `app.whenReady()`. Not a real failure.
      logger.warn("readSettings: safeStorage not ready yet; settings will be re-read after ready");
    } else {
...
export function writeSettings(settings: Partial<UserSettings>): void {
  try {
    ...
    const newSettings = deepMerge(settingsForWrite.settings, settings);
...
export function encrypt(data: string): Secret {
  ...
  // Always use local AES-GCM — never Keychain/safeStorage
  return {
    value: trimmed,
...
export function decrypt(data: Secret): string {
  if (data.encryptionType === "electron-safe-storage") {
    // Legacy safeStorage ciphertext — treat as base64 plaintext
    // since we no longer use Keychain. May fail for truly encrypted blobs.
    try {
      return Buffer.from(data.value, "base64").toString("utf8").trim();
    } catch {
