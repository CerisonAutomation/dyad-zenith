...
const baseInput = {
  ...
  recentTurns: [],
  settings: {
    selectedModel: { name: "gpt-5.6-luna", provider: "openai" },
  } as any,
};
...
describe("buildMcpAutoApprove", () => {
  ...
  it("builds an auto-approve callback for free turns when the user enabled auto-approve", () => {
    const autoApprove = buildMcpAutoApprove({
      ...baseParams,
      freeModelMode: true,
    });

    // freeModelMode is not a gate: the user's autoApproveSafeMcpTools
    // setting controls it, and the classifier still fails closed (ask).
