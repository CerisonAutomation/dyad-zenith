...
// ============================================================================
// Consolidated `git` tool — ONE registry entry replaces the six git_* tools.
// The individual tools remain exported for tests and internal dispatch; the
// merged tool is what the model sees, cutting five schemas from the prompt.
// ============================================================================

const gitCommandSchema = z.enum([
  "status",
  "diff",
  "log",
  "show_commit",
...
const gitSchema = z.object({
  command: gitCommandSchema.describe(
    "Which git operation to run: status (working tree), diff (changes), log (history), show_commit (commit details), show_file (file contents at a revision), restore_file (discard changes to a file)",
  ),
  scope: z
    .enum(["unstaged", "staged", "all"])
    .optional()
    .describe(
      "For diff: unstaged compares index to working tree; staged compares HEAD to index; all compares HEAD to working tree (default: all)",
    ),
  path: pathSchema.optional().describe("Literal path relative to the app root (diff, show_commit, show_file, restore_file)"),
  revision: revisionSchema.optional().describe("Commit hash, branch, or tag (log, show_commit, show_file, restore_file)"),
  max_count: z
    .number()
    .int()
    .min(1)
    .max(100)
    .optional()
    .describe("For log: maximum number of commits to return (default 20)"),
  start_line_one_indexed: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe("For show_file: one-indexed start line (inclusive)"),
  end_line_one_indexed_inclusive: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe("For show_file: one-indexed end line (inclusive)"),
});

export const gitTool: ToolDefinition<z.infer<typeof gitSchema>> = {
  name: "git",
  description:
    "Inspect the app's Git repository in one call: status (working tree), diff (changes), log (history), show_commit (commit details), show_file (file at a revision), restore_file (discard changes). Choose the command and pass only its params.",
  inputSchema: gitSchema,
  defaultConsent: "always",
  buildXml: (args, isComplete) =>
    buildGitPreview(args.command ?? "status", args, isComplete),
  execute: async (args, ctx) => {
    switch (args.command) {
      case "status":
        return gitStatusTool.execute({}, ctx);
      case "diff":
        return gitDiffTool.execute(
          { scope: args.scope, path: normalizeGitFilterPath(args.path) },
          ctx,
        );
      case "log":
        return gitLogTool.execute(
          { revision: args.revision, max_count: args.max_count },
          ctx,
        );
      case "show_commit":
        if (!args.revision) {
          throw new DyadError(
            "git show_commit requires a revision",
            DyadErrorKind.Validation,
          );
        }
        return gitShowCommitTool.execute(
          { revision: args.revision, path: normalizeGitFilterPath(args.path) },
          ctx,
        );
      case "show_file":
        if (!args.revision || !args.path) {
          throw new DyadError(
            "git show_file requires a revision and a path",
            DyadErrorKind.Validation,
          );
        }
        return gitShowFileTool.execute(
          {
            revision: args.revision,
            path: args.path,
      ...
      case "restore_file":
        if (ctx.readOnly || ctx.planModeOnly) {
          throw new DyadError(
            "git restore_file is not available in read-only or plan mode",
            DyadErrorKind.Validation,
          );
        }
        if (!args.revision || !args.path) {
          throw new DyadError(
            "git restore_file requires a revision and a path",
            DyadErrorKind.Validation,
          );
        }
        return gitRestoreFileTool.execute(
          { revision: args.revision, path: args.path },
          ctx,
        );
    }
  },
