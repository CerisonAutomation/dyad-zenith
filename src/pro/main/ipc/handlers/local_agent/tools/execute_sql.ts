...
function postgresErrorHint(query: string, error: unknown): string {
  const message = error instanceof Error ? error.message : String(error);
  const hints: Array<[RegExp, string]> = [
    [/42703/, " Column does not exist — verify the exact column name (\"information_schema.columns\") before retrying; do not guess."],
    [/42P01/, " Table does not exist — verify the table name (\"information_schema.tables\") before retrying."],
    [/42601/, " SQL syntax error — check quotes, commas, and statement structure; validate the statement in isolation."],
    [/23514/, " A CHECK constraint rejected the data — existing rows violate the new constraint; handle or clean them first."],
    [/23505/, " Unique constraint violation — the row already exists; use ON CONFLICT or a different key."],
    [/23503/, " Foreign key violation — the referenced row does not exist; insert it first."],
  ];
...
export const executeSqlTool: ToolDefinition<z.infer<typeof executeSqlSchema>> =
  {
    ...
    execute: async (args, ctx: AgentContext) => {
      ...
      if (ctx.supabaseProjectId) {
        ...
        try {
          sqlResult = await executeSupabaseSql({
          ...
          throw new DyadError(
            `${error instanceof Error ? error.message : String(error)}${postgresErrorHint(args.query, error)}`,
