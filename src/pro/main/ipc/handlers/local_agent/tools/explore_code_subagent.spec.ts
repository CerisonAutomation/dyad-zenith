...
describe("runExploreCodeSubagent", () => {
  ...
  it("runs when Dyad Pro is unavailable but a custom model is configured (BYOK-first)", async () => {
    mocks.readSettings.mockReturnValue({
      selectedModel: { provider: "openai", name: "gpt-4" }, enableDyadPro: false });
    const report = await runExploreCodeSubagent({
      args: { query: "widget save flow", intent: "locate" },
      ctx: createMockContext(),
    });
...
function createMockContext(appPath = "/tmp/app"): AgentContext {
  return {
    ...
    localAgentSettings: {
      selectedModel: { provider: "openai", name: "value" },
    },
