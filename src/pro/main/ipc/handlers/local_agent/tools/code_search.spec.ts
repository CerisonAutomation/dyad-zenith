...
const extractCodebaseMock = vi.hoisted(() => vi.fn());
vi.mock("@/utils/codebase", () => ({
  extractCodebase: extractCodebaseMock,
}));
extractCodebaseMock.mockResolvedValue({ files: [] });
...
describe("codeSearchTool", () => {
  ...
  describe("isEnabled", () => {
    ...
    it("is enabled for non-Pro when the explorer is unavailable (BYOK-first)", () => {
      mocks.readSettings.mockReturnValue({
      enableCodeExplorer: false });
      mocks.isCodeExplorerReady.mockReturnValue(false);
  ...
  describe("execute - app_name (referenced apps)", () => {
    it("routes to the referenced app's path when app_name matches", async () => {
      ...
      // Engine fetch was removed: local extraction must target the
      // referenced app's directory.
      const extractCall = extractCodebaseMock.mock.calls.find(
        (c) => c[0] && (c[0] as { appPath?: string }).appPath === otherAppDir,
      );
      expect(extractCall).toBeDefined();
      // The referenced app's directory is the extraction root — the search
      // runs over the referenced app, not the current one.
      const firstCall = extractCodebaseMock.mock.calls[0];
      expect((firstCall[0] as { appPath?: string }).appPath).toBe(otherAppDir);
      // Local ranking (extraction mock returns no files) still completes.
      expect(extractCodebaseMock).toHaveBeenCalled();
    });
