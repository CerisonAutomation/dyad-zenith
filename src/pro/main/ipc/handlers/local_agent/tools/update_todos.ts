...
const updateTodosSchema = z.object({
  merge: z
    ...
    .default(true)
    .describe(
      "Whether to merge the todos with the existing todos. If true, the todos will be merged into the existing todos based on the id field. You can leave unchanged properties undefined. If false, the new todos will replace the existing todos. Defaults to true.",
    ),
  todos: z
    .array(todoSchema)
    .describe(
