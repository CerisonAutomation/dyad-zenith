...
// Custom build: model is resolved dynamically from settings.selectedModel at runtime.
...
export async function runExploreCodeSubagent({
  ...
  const maxOutputTokens =
    runtime.maxOutputTokens ?? SUBAGENT_MAX_OUTPUT_TOKENS;
...
async function collectRawExploreObservation({
  ...
  const effectiveArgs = await normalizeExploreCodeArgsForApp({
  ...
  return {
    ...
    execute: async (toolArgs: RawExploreCodeArgs) => {
      ...
      try {
        ...
        const effectiveToolArgs = await normalizeExploreCodeArgsForApp({
          appPath: targetAppPath,
          args: lockedArgs,
        });
        const rawResult = await runRawExploreCode({
