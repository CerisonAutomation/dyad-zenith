...
export async function normalizeExploreCodeArgsForApp<
...
>({
  ...
  return {
    ...args,
    tsconfig_path:
      (await validAppRelativeTsconfigPath(appPath, args.tsconfig_path)) ??
      (fallbackTsconfigPath
        ? await validAppRelativeTsconfigPath(appPath, fallbackTsconfigPath)
        : undefined) ??
      undefined,
  } as TArgs;
}

async function validAppRelativeTsconfigPath(
  appPath: string,
...
export async function runRawExploreCode({
  ...
  const effectiveArgs = await normalizeExploreCodeArgsForApp({ appPath, args });
