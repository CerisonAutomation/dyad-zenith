...
describe("local-agent Git tool definitions", () => {
  ...
  it("includes read tools in normal, ask, and plan modes but gates restore", () => {
    ...
    // The consolidated `git` tool stays available in read-only/plan mode for
    // read commands; restore_file is gated internally by the dispatcher.
    expect(gitTool.modifiesState).toBeFalsy();
    expect(shouldIncludeTool(gitTool, ctx)).toBe(true);
    expect(shouldIncludeTool(gitTool, ctx, { readOnly: true })).toBe(true);
    expect(shouldIncludeTool(gitTool, ctx, { planModeOnly: true })).toBe(true);
    expect(getDefaultConsent("git")).toBe("always");
  });
  ...
  it("applies the app-blueprint gate before restoring", async () => {
    ...
    await expect(
      toolSet.git.execute?.(
        { command: "restore_file", revision: "HEAD", path: "file.txt" },
        {} as never,
      ),
