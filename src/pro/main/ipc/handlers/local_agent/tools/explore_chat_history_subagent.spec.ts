...
describe("runExploreChatHistorySubagent", () => {
  ...
  it("runs for non-Pro contexts when a custom model is configured (BYOK-first)", async () => {
    // makeAgentContext defaults to isDyadPro: false; the Pro gate was removed
    // in favor of resolveAgentModelRuntime (explicit non-auto model required).
  ...
  it("runs when settings do not enable Dyad Pro but a custom model is set", async () => {
    mocks.readSettings.mockReturnValue({
      selectedModel: { provider: "openai", name: "gpt-4" },
      enableDyadPro: false,
      providerSettings: {
  ...
  it("runs without an auto-provider key when a custom model is configured", async () => {
    mocks.readSettings.mockReturnValue({
      selectedModel: { provider: "openai", name: "gpt-4" },
      enableDyadPro: true,
      providerSettings: {},
