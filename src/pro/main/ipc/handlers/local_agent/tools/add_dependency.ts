  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    try {
      ...
      if (
        ...
        /ELIFECYCLE|exit code 1/i.test(error.message)
      ) {
        throw new DyadError(
          `${error.message} — Dependency install failed (exit code 1). ` +
            "Common causes: network/registry unreachable, low system memory, or a version conflict. " +
            "Verify network, free memory, then retry the install.",
