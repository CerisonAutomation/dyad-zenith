...
export interface AgentContext {
   ...
   */
  isDyadPro: boolean;
  /** True when the turn runs in ask/read-only mode; state-modifying tools
   * are filtered out by shouldIncludeTool, and merged dispatcher tools use
   * this to reject mutating sub-commands (e.g. git restore_file). */
  readOnly?: boolean;
  /** True when the turn runs in plan mode (plan-only tool set). */
...
export interface ToolDefinition<T = any> {
  ...
  /** Alternative names the model may use to call this tool (e.g. camelCase variants). */
