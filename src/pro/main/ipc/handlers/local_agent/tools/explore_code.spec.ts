...
describe("exploreCodeTool", () => {
  beforeEach(() => {
    ...
    mocks.readSettings.mockReturnValue({
      selectedModel: { provider: "openai", name: "gpt-4" }, enableCodeExplorer: true });
  ...
  it("is not engine-gated (available to BYOK/free turns alike)", () => {
    // The engine-endpoint concept was removed; the tool must not require it.
    expect((exploreCodeTool as unknown as Record<string, unknown>).usesEngineEndpoint).toBeUndefined();
  });
