...
`,
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (args.old_string === args.new_string) {
      throw new DyadError(
        "old_string and new_string must be different. This usually means the edit is already applied, " +
          "or the file content differs from what you expect — re-read the file with read_file and use " +
          "the exact current text as old_string.",
    ...
    await withLock(getFileWriteKey(fullFilePath), async () => {
      ...
      if (!result.success || typeof result.content !== "string") {
        ...
        let guidance = "";
        if (/multiple locations/i.test(rawError)) {
          guidance =
            " The search text matches multiple places in the file. Include MORE surrounding context lines (3-5 lines before AND after the change) to uniquely identify the target location.";
        } else if (/did not match/i.test(rawError)) {
          guidance =
            " The search text does not match any content in the file. Re-read the file with read_file to see its current content, then use the EXACT text (including indentation and whitespace) as old_string. The file may have been modified since you last read it.";
        } else if (/empty.*SEARCH/i.test(rawError)) {
          guidance =
            " The search block is empty. Provide the exact lines of code you want to replace.";
        }
        throw new Error(
          `search_replace failed: ${rawError}.${guidance}`,
        );
      }
