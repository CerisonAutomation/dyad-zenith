...
function buildSandboxFailureMessage(params: {
  ...
  const isProtectedPath =
    /protected path/i.test(params.errorMessage) ||
    /node_modules/i.test(params.errorMessage);
  const isVarError = /only let and const are supported/i.test(params.errorMessage);
  const isRegexFlagError = /Unexpected flag.*regular expression/i.test(params.errorMessage);
  const isAwaitError = /await is only allowed within async/i.test(params.errorMessage);
  const mentionsShellFn =
    /run_shell_command|shell_exec|execSync|child_process|\.exec\(|spawn\(|system\(|child_process/i.test(
      `${params.script} ${params.errorMessage}`,
    );

  const hints: string[] = [];
  if (mentionsShellFn) {
    hints.push(
      "Hint: the sandbox has NO shell access — run_shell_command, exec, spawn and child_process do not exist here. " +
        "The sandbox only provides: read_file(path), list_files(dir), file_stats(path), write_file(path, content), check_package_installed(pkg). " +
        "For git commands (remote, log, status, diff, show), use the dedicated 'git' tool instead — never the sandbox. " +
        "For other terminal work use the matching dedicated tool (run_type_checks, run_tests, build_app, execute_sql) — there is no generic shell tool."
    );
  }
  if (isProtectedPath) {
    hints.push(
      "Hint: the sandbox blocks node_modules, .git and other protected paths. " +
        "To check if a package is installed, use the built-in check_package_installed() host function: " +
        "e.g. `const result = await check_package_installed(\"react\"); result.installed` " +
        "It reads package.json safely — no node_modules access needed. " +
        "For raw deps, use read_file(\"package.json\") and parse the JSON."
    );
  }
  if (isVarError) {
    hints.push(
      "Hint: the sandbox only supports 'let' and 'const' — not 'var'. " +
        "Replace all 'var' with 'let' or 'const'."
    );
  }
  if (isRegexFlagError) {
    hints.push(
      "Hint: the sandbox regex engine only supports basic patterns. " +
        "Remove regex flags (like /g, /i, /c) or use String methods instead " +
        "(e.g. str.includes(), str.split(), str.match())."
    );
  }
  if (isAwaitError) {
    hints.push(
      "Hint: 'await' is only allowed at the top level or inside async functions. " +
        "Wrap the code in an async IIFE: (async () => { ... })()"
    );
  }

  return [
    "The sandbox script failed:",
    "",
    "Script:",
    params.script,
    "",
  ...
  const payload = JSON.stringify(
    {
      ...
      ...(params.autoFixes && params.autoFixes.length > 0
        ? { autoFixes: params.autoFixes }
        : {}),
    },
    null,
    2,
...
/** Check if an npm package is listed in package.json. Preferred over file_stats("node_modules/...") which is blocked. */
declare function check_package_installed(packageName: string): Promise<PackageInstallStatus>;
...
/**
 * Pre-validate and auto-fix common MustardScript patterns.
 * Catches issues before mustardscript parsing to give clearer errors
 * and auto-fix what's safe to fix.
 *
 * Returns { script, autoFixes } where script is the (possibly patched) script
 * and autoFixes is a list of what was changed.
 */
function prevalidateScript(script: string): {
  script: string;
  autoFixes: string[];
} {
  let fixed = script;
  ...
  // 1. Convert var → let (safe: var and let have same scoping in MustardScript)
  const varCount = (fixed.match(/\bvar\s+/g) || []).length;
  if (varCount > 0) {
    fixed = fixed.replace(/\bvar\s+/g, "let ");
    autoFixes.push(`Converted ${varCount} var → let`);
  }

  // 2. Strip regex flags (MustardScript doesn't support them)
  const regexFlagCount = (fixed.match(/\/[^/]+\/([gimsuy]+)/g) || []).length;
  if (regexFlagCount > 0) {
    fixed = fixed.replace(/\/([^/]+)\/([gimsuy]+)/g, "/$1/");
    autoFixes.push(
      `Stripped regex flags from ${regexFlagCount} regex literals (MustardScript doesn't support flags)`,
    );
  }

  // 3. Detect top-level await outside async function
  const hasAsyncFn = /\basync\s*(function|\()/g.test(fixed);
  const topLevelAwaitCount = (
    fixed.match(/^\s*(const|let)\s+\w+\s*=\s*await\s/gm) || []
  ).length;
  if (topLevelAwaitCount > 0 && !hasAsyncFn) {
    autoFixes.push(
      `Warning: ${topLevelAwaitCount} top-level await(s) detected. MustardScript supports top-level await, but if you get "await is only allowed within async" errors, wrap the code in an async function.`,
    );
  }

  if (autoFixes.length > 0) {
    console.log(`[MustardScript pre-validator] ${autoFixes.join("; ")}`);
  }

  return { script: fixed, autoFixes };
}
...
export const executeSandboxScriptTool: ToolDefinition<ExecuteSandboxScriptArgs> =
  {
    ...
    description:
      "Run a sandboxed JavaScript program. Supports file inspection, file writes, and MCP tool calls. " +
      "CONSTRAINTS: (1) node_modules/.git BLOCKED — use check_package_installed(\"pkg\") or read_file(\"package.json\") to inspect deps. " +
      "(2) Only 'let' and 'const' — NOT 'var'. (3) No regex flags (/g, /i, /c) — use String methods " +
      "(includes, split, replace, indexOf). (4) No imports/require — use host functions (read_file, " +
      "list_files, grep, execute_sql, web_search, etc.) (5) No object-literal statement expressions — " +
      "assign to variable and return it. (6) Keep scripts small and single-purpose.",
    ...
    execute: async (args: ExecuteSandboxScriptArgs, ctx: AgentContext) => {
      ...
      // Pre-validate and auto-fix common MustardScript mistakes before
      // execution so the agent doesn't burn turns on syntax errors.
      const { script: validatedScript, autoFixes } = prevalidateScript(
        args.script,
      );
      try {
        const result =
          executionThread === "worker"
            ...
            : await runInMainThread({
                args: { ...args, script: validatedScript },
                ctx,
                observeHostCall,
              });
