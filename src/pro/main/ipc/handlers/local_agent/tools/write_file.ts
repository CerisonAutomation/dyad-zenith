...
export const writeFileTool: ToolDefinition<z.infer<typeof writeFileSchema>> = {
  name: "write_file",
  description: "Create or completely overwrite a file in the codebase. Use for new files or when rewriting most of an existing file. For partial edits to existing files, prefer search_replace.",
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    await withLock(getFileWriteKey(fullFilePath), async () => {
      ...
      // Write file content
      await fs.promises.writeFile(fullFilePath, args.content);
