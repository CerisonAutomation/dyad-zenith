...
// Model is resolved dynamically from settings.selectedModel.
...
// the feature is off. The classifier uses the user-selected Settings model and
// fails closed to a consent prompt if that model is unavailable. Shared by both
...
export function buildMcpAutoApprove(params: {
  ...
  if (!params.settings.autoApproveSafeMcpTools) {
    return undefined;
  }
  return async () => {
