...
function buildTestSettings(
  ...
  const baseSettings = {
    selectedModel: overrides.selectedModel
        ? { provider: "openai", name: overrides.selectedModel }
  ...
  TOOL_DEFINITIONS: [
    {
      ...
      buildXml: (args: { chat_id?: number }, isComplete: boolean) =>
        args.chat_id && !isComplete
          ...
          : undefined,
      execute: async () => "mock chat content",
    },
    {
      name: "write_file",
      buildXml: (
...
describe("handleLocalAgentStream", () => {
  describe("recoverTextEmittedToolCalls", () => {
    it("executes <dyad-read> tags from a text-only final step and returns results", async () => {
      ...
      const ctx = {
        ...
        todos: [],
        dyadRequestId: "test-recovery",
        fileEditTracker: {},
        isDyadPro: true,
        onXmlStream: () => {},
        onXmlComplete: () => {},
        requireConsent: () => Promise.resolve({ approved: true }),
        appendUserMessage: () => {},
        onUpdateTodos: () => {},
      } as never;
      const { executed, feedbackMessage } = await recoverTextEmittedToolCalls({
        steps: [
          {
            toolCalls: [],
            content: [
              {
                type: "text",
                text: `Let me read the chat.<dyad-read-chat chat-id="123"></dyad-read-chat>`,
              },
            ],
          } as any,
        ],
      ...
      expect(feedbackMessage).toContain("mock chat content");
      expect(feedbackMessage).toContain("Continue the task");
    });

    it("does nothing for steps with structured tool calls", async () => {
      const { event } = createFakeEvent();
      const ctx = {
        event: event,
        appId: 1,
        ...
        steps: [
          {
            ...
            content: [
              { type: "text", text: `<dyad-read-chat chat-id="1"></dyad-read-chat>` },
            ],
          } as any,
        ],
        ctx: ctx as any,
        ...
        steps: [
          {
            ...
            content: [
              {
                type: "text",
                text: `<dyad-write path="out.txt" content="hacked"></dyad-write>`,
              },
            ],
          } as any,
        ],
  ...
  describe("Pro status validation", () => {
    it("should send error when no custom agent model is configured", async () => {
      // Arrange
      const { event, getMessagesByChannel } = createFakeEvent();
      // No custom model configured (auto) and no Pro entitlement:
      // the agent-model guard must reject before streaming.
      mockSettings = {
        ...buildTestSettings({ enableDyadPro: false }),
      ...
      expect(errorMessages[0].args[0]).toMatchObject({
        chatId: 1,
        error: expect.stringContaining("requires a custom provider model"),
      });
    });
  ...
  describe("Synthetic plan_or_execute reflection", () => {
    it("injects a non-persisted reflection message after invalid plan_or_execute input", async () => {
      // Arrange
      const { event } = createFakeEvent();
      mockSettings = buildTestSettings({ enableDyadPro: true });
      mockChatData = buildTestChat({
          ...
          fullStream: (async function* () {
            ...
            await options.onStepFinish?.({
              content: [
                {
                  type: "tool-error",
                  toolName: "plan_or_execute",
                  toolCallId: "call_plan_q",
                  input: invalidQuestionnaireInput,
                  error:
                    "Invalid input for tool plan_or_execute: questions[0].question is required",
                },
              ],
        ...
        (message: any) =>
          ...
          message.content.some(
            (part: any) =>
              ...
              typeof part.text === "string" &&
              part.text.includes(
                "plan_or_execute tool call had a format error",
              ),
          ),
      );
  ...
  describe("Todo follow-up", () => {
    ...
    it("runs a follow-up pass when the first pass ends with set_chat_summary and incomplete todos remain", async () => {
      ...
      // New behavior: the follow-up pass still runs, but NO user-role reminder
      // message is injected (it confused the model by looking like a real user
      // message). The todo state is already in context from update_todos results.
      const hasInjectedTodoReminder = secondPassMessages.some(
        (message: any) =>
          message.role === "user" &&
          Array.isArray(message.content) &&
          message.content.some(
      ...
      expect(hasInjectedTodoReminder).toBe(false);
    });
  });
