...
import { capturePublicWebScreenshot, fetchWebPage } from "./local_agent/tools/direct_web";
...
export function registerThemesHandlers() {
  ...
  handle(
    ...
    async (): Promise<ThemeGenerationModelOption[]> => {
      ...
      if (!selected?.provider || !selected?.name || selected.provider === "auto") {
        return [];
      }
      return [
        {
          id: `${selected.provider}:${selected.name}`,
          label: `${selected.provider} / ${selected.name}`,
        },
      ];
    },
  );
...
images: ${imagesPart}`;
  ...
  handle(
    ...
    async (
      ...
      // Crawl locally through the same hardened direct-web layer used by the
      // Dyad web_fetch/web_crawl tools. No provider credential crosses this
      // boundary and no hosted Pro engine is involved.
      logger.log(`Crawling website for theme: ${params.url}`);
      const page = await fetchWebPage(params.url);
      ...
      logger.log(`Website crawled successfully: ${page.url}`);
