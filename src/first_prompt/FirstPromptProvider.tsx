...
export function useFirstPromptController(): FirstPromptController {
  ...
  if (!context) {
    // During HMR or race conditions the provider may not yet be mounted.
    // Return a no-op controller instead of throwing to prevent cascading
    // error-boundary resets that break the entire page.
    console.warn("useFirstPromptController called outside FirstPromptProvider — returning no-op controller");
    return {
      getSnapshot: () => ({ type: "idle" as const }),
      send: () => false,
    } as unknown as FirstPromptController;
  }
...
export function useFirstPromptProviderResume(): (
  ...
  const context = useContext(FirstPromptContext);
  if (!context) {
    console.warn("useFirstPromptProviderResume called outside FirstPromptProvider");
    return () => {};
  }
