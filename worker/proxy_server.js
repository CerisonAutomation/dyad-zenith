...
let dyadPreviewNavigatorContent = null;
...
// Load preview navigator (browser-automation in preview iframe)
try {
  const dyadPreviewNavigatorPath = path.join(__dirname, "dyad-preview-navigator.js");
  dyadPreviewNavigatorContent = fs.readFileSync(dyadPreviewNavigatorPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] dyad-preview-navigator.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read dyad-preview-navigator.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  if (dyadPreviewNavigatorContent) {
    scripts.push(`<script>${dyadPreviewNavigatorContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] dyad-preview-navigator.js was not injected.");</script>',
    );
  }
