...
// Runtime allowlist: Electron Forge's Vite output plus non-bundled runtime assets only.
// Returning true means "ignore". This intentionally prevents tests, source-only
// tooling, local config, and user/runtime state from leaking into packaged apps.
const ignore = (file: string) => {
  if (!file) return false;
  if (file === "/node_modules") return false;
  if (file.startsWith("/drizzle")) return false;
  if (file.startsWith("/scaffold")) return false;
  // Main BrowserWindow resolves its Linux/runtime icon from app.getAppPath().
  if (file.startsWith("/assets/icon")) return false;
  ...
  if (isPgRuntimeDependency(file)) return false;
  if (file.startsWith("/.vite")) return false;
  return true;
};

const isEndToEndTestBuild = process.env.E2E_TEST_BUILD === "true";
const isUnsignedBuild = process.env.DYAD_UNSIGNED_BUILD === "true";
const skipMacSigning = isEndToEndTestBuild || isUnsignedBuild;
...
const config: ForgeConfig = {
  ...
  plugins: [
    ...
    new VitePlugin({
      // Bound concurrent Vite jobs: this app has multiple main/preload/worker
      // targets and unbounded parallel builds can spike memory in CI/desktops.
      concurrent: 2,
    ...
    new FusesPlugin({
      ...
      // Flipping fuses mutates the Electron binary. Unsigned Apple Silicon
      // packages need a fresh ad-hoc signature afterwards or macOS can reject
      // the app before it starts. Signed release builds are signed normally.
      resetAdHocDarwinSignature:
        skipMacSigning && process.platform === "darwin" && process.arch === "arm64",
