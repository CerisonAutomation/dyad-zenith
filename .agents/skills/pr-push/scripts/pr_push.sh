...
run_checks() {
  ...
  bun run fmt || die "Formatter failed; fix the issues above and rerun"

  log "Running lint fix"
  bun run lint:fix || die "Lint failed; fix the issues above and rerun"

  log "Running typecheck"
  bun run ts || die "Typecheck failed; fix the issues above and rerun"
}
