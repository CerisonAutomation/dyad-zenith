...
function bumpVersion(newVersion) {
  ...
  // Bun's text lockfile tracks dependency resolution, not the root package version.
  // Keep version ownership in package.json and leave bun.lock untouched.
  ...
  const prUrl = run("gh", [
    "pr", "create", "--title", `Bump to ${tag}`, "--body", "#skip-bb",
  ]).trim();
