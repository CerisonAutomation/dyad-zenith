...
function generateWebServerConfigs(): PlaywrightTestConfig["webServer"] {
  return Array.from({ length: workerCount }, (_, index) => {
    ...
    const command =
      index === 0
        ? `cd testing/fake-llm-server && bun run build && bun run start -- --port=${port}`
        : `while ! curl -fsS http://localhost:${FAKE_LLM_BASE_PORT}/health >/dev/null; do sleep 1; done; cd testing/fake-llm-server && bun run start -- --port=${port}`;
